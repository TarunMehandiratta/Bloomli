...
class DiscoverCategoriesAdapterTwo(var context : Context) : RecyclerView.Adapter<RecyclerView.ViewHolder>(){
    var selectedPosition = -1
    class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView){}

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        val view =
            LayoutInflater.from(parent.context).inflate(layout.discover_categories_adapter_two, parent, false)
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        val selectedPosition2 = position
        if (selectedPosition2==0){
            holder.itemView.cate_adapter_text.setText("hapiness")
        }
        else if (selectedPosition2==1){
            holder.itemView.cate_adapter_text.setText("Religion")
            holder.itemView.icon.setImageResource(R.drawable.heart_tittle)

        }
        else if (selectedPosition2==2){
            holder.itemView.cate_adapter_text.setText("Sex & Relationships")
            holder.itemView.icon.setImageResource(R.drawable.smile)
        }
